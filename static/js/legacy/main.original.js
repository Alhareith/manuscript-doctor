...
const operationNames = {
    ...
    document_prepare: ["تصحيح الميل والاقتصاص تلقائيًا", "Guided Preparation"]
...
function renderParameterFields(operationId) {
    ...
    if (!fields.length) {
        ...
        note.textContent = operationId === "document_prepare"
            ? "سيختار النظام Guided أولًا ثم Region عند الحاجة، وينفذ Perspective Crop وDeskew ثم يعرض النتيجة للاعتماد أو الرفض."
...
function bindEvents() {
    ...
    elements.applyManualButton?.addEventListener("click", () => {
        if (state.manualPreviewCandidate) {
            approveManualOperation();
        } else {
            applyManualOperation({ live: true });
        }
    });
