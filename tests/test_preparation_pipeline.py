...
def test_precomputed_preparation_plan_skips_boundary_and_skew_detection(monkeypatch):
    image = np.full((800, 1200, 3), 220, dtype=np.uint8)

    def forbidden_boundary(_image):
        raise AssertionError("boundary detector must not run for a precomputed plan")

    monkeypatch.setattr(
        preparation_pipeline,
        "detect_skew",
        lambda _image: (_ for _ in ()).throw(
            AssertionError("skew detector must not run for a precomputed plan")
        ),
    )
    ...
    boundary = {
        ...
        "status": "accept_automatic",
        "method_used": "bright",
        "corners": [[60, 40], [540, 45], [535, 360], [65, 355]],
        "confidence": 0.82,
        "area_ratio": 0.64,
        "reason": "accepted: preview plan",
    }
    skew = {
        "angle": 0.0,
        "confidence": 0.95,
        "line_count": 12,
        "dispersion": 0.2,
        "reason": "measured: preview plan",
        "detection_scale": 1.0,
        "detection_dimensions": {"width": 480, "height": 315},
    }

    result = prepare_document(
        image,
        boundary_detector=forbidden_boundary,
        precomputed_boundary=boundary,
        precomputed_skew=skew,
        plan_source_dimensions={"width": 600, "height": 400},
    )

    assert result["prepared"] is True
    assert result["boundary"]["reused_from_preview"] is True
    assert result["skew"]["reused_from_preview"] is True
    assert result["boundary"]["corners"] == [
        [120, 80],
        [1080, 90],
        [1070, 720],
