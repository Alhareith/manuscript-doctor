...
def build_preview_payload(image):
    """Encode an in-memory preview; never writes a result file."""
...
def build_result_metadata(result_id, image, source_image_id):
    return {
        "id": result_id,
...
def save_manual_result(image, folder, source_image_id, parent_result_id=None):
...
def create_app(test_config=None):
    ...
    def apply_manual_operation(image_id):
        ...
        return success_response(
            data={
                "result": (build_result_metadata(result_id, processed, image_id)),
    ...
    def preview_manual_operation(image_id):
        ...
        if source_result_id:
            ...
            if source_manifest.get("kind") != "manual_approved":
                return error_response(
                    "INVALID_SOURCE_RESULT_KIND",
                    "النتيجة المصدرية ليست نتيجة يدوية معتمدة.",
                    400,
                )
        ...
        try:
            ...
            preview = build_preview_payload(processed)
        except (ValueError, TypeError) as error:
            return error_response(
                "INVALID_OPERATION_PARAMETERS",
                "Parameters العملية غير صالحة.",
    ...
    def run_pipeline(image_id):
        ...
        try:
            ...
            pipeline_result = run_smart_pipeline(original, analysis)
        ...
        try:
            result_id, _ = save_png_result(final_image, result_folder)
        ...
        for candidate in pipeline_result["binarization_candidates"]:
            ...
            try:
                candidate_id, _ = save_png_result(candidate_image, result_folder)
            ...
            binarization_results.append(
                {
                    "result": (
                        build_result_metadata(candidate_id, candidate_image, image_id)
                    ),
        ...
        return success_response(
            data={
                "result": (build_result_metadata(result_id, final_image, image_id)),
