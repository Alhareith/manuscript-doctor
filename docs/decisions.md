...
## DEC — Recommendation Engine Rule-Based

**الحالة:** معتمد

### القرار

سيستخدم Recommendation Engine قواعد صريحة قابلة للتفسير مبنية على Diagnosis وPreservation Profile ونتائج Operation Evaluation.

### السبب

حجم المشروع وطبيعته التعليمية لا يتطلبان Machine Learning لاتخاذ قرار يمكن تمثيله بقواعد واضحة.

---

## DEC — عدم السماح لكل العمليات بالدخول إلى التوصية التلقائية

**الحالة:** معتمد

### القرار

تدخل العمليات التالية فقط بصورة مشروطة في التوصية:

- CLAHE
- Median Denoising
- Sharpening
- Adaptive Threshold ضمن Binarization

تبقى العمليات الأخرى يدوية أو محدودة حسب نتائج التقييم.

### السبب

أظهرت Phase 8 اختلافًا واضحًا في المخاطر والقدرة على التعميم بين العمليات.

---

## DEC — أقل معالجة مفيدة هي الأفضل مبدئيًا

**الحالة:** معتمد

### القرار

يستخدم Recommendation Engine الإعدادات المحافظة التي أظهرت فائدة مع تغير بنيوي أقل، بدل اختيار الإعداد الأعلى تأثيرًا.
...
- Median `kernel=3`
- Sharpen `amount=0.25`
- Adaptive Threshold `35/11`

---

## DEC — فصل Enhancement عن Binarization

**الحالة:** معتمد

### القرار

تعامل عمليات Binarization كمخرجات معالجة ذات هدف مستقل ولا تستبدل تلقائيًا نتيجة Enhancement الأساسية.
