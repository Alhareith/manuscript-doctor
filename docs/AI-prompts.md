...
> **الحالة الزمنية:** هذا الملف سجل تاريخي لمنهج التفكير والبرومبتات المستخدمة أثناء مراحل التحليل والتنفيذ. بعض البرومبتات تصف بدائل أو خططاً لم تُنفذ كما هي، لذلك لا يُستخدم هذا الملف لإثبات وجود وظيفة. المرجع الحاسم للتنفيذ الحالي هو `docs/current-baseline.md`، وللتحقق هو `manual-qa-report.md`.
...
## ملحق القراءة الحالية — ما الذي أصبح منفذاً؟

تُقرأ البرومبتات السابقة بوصفها سجلّاً لتطور التفكير: من رفض محرر فلاتر عام، إلى بناء مسار **Diagnose → Treat → Preserve → Verify**، ثم فصل Manual عن Smart، ثم توثيق الاختبارات والقرارات. لا يعني وجود Prompt عن وظيفة أن الوظيفة أضيفت كاملة أو بقيت بتفاصيلها الأولى.

| القرار الموثق في البرومبتات | التفسير الحالي |
| --- | --- |
| Manual مقابل Smart | المعاينة اليدوية والاعتماد يدعمان سلسلة يدوية مقصودة، بينما Smart Pipeline يطبق سياسة قبول وبوابات تحقق مستقلة. |
| Original immutable | الأصل يبقى مصدراً غير معدل، وتستخدم الخطوات اللاحقة نتيجة معتمدة سابقة عبر `source_result_id`. |
| Frontend مقابل Backend | Canvas يعرض Preview خفيفاً، لكن Flask/OpenCV ينشئ artifact النهائي عند الاعتماد. |
| Preservation | يقيس أثر المعالجة على بنية الصورة، ولا يدعي اكتشاف فقد حرف أو نسبة حفظ للنص دون OCR. |
| Super Resolution | أضيفت كعملية يدوية مستقلة بتكبير Lanczos وUnsharp على luminance؛ لا تدخل Smart Pipeline ولا تستعيد معلومات مفقودة بالكامل. |
| النطاق | لا توجد حالياً OCR أو DNN أو Generative Restoration أو Database أو Queue أو Docker ضمن التنفيذ. |

لذلك يبقى هذا الملف مناسباً لشرح **كيف اتُّخذت القرارات**، بينما يجب الرجوع إلى `docs/architecture.md` لمعرفة البنية، و`docs/decisions.md` لمعرفة أسباب القرارات النهائية، و`manual-qa-report.md` لمعرفة ما تم اختباره فعلياً.
